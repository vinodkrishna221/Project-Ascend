...
// Comprehensive fix script for all build errors
const fs = require('fs');
...
if (fs.existsSync(monitoringPath)) {
  ...
  const lines = monitoringContent.split('\n');
  let braceCount = 0;
  let inFunction = false;
  
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('const fetchDashboardData = useCallback(async () => {')) {
      inFunction = true;
      braceCount = 1;
      continue;
    }
...
if (fs.existsSync(verifyEmailPath)) {
  ...
  // Find and fix the handleSubmit function closing
...
if (fs.existsSync(complianceMonitorPath)) {
  ...
  complianceContent = complianceContent.replace(
    /private isProduction: boolean;\s*\n/,
    ''
  );
  
  complianceContent = complianceContent.replace(
    /this\.isProduction = process\.env\.NODE_ENV === 'production';\s*\n/,
    ''
  );
  
  // Fix unused userId parameters by adding underscore prefix
  complianceContent = complianceContent.replace(
...
class AnalyticsService {
  async getAnalytics(timeRange: '7d' | '30d' | '90d' = '30d'): Promise<AnalyticsData> {
    ...
    return {
      ...
      topCommunities: [
        { name: 'Computer Science', members: 450, posts: 1200 },
        { name: 'Data Science', members: 320, posts: 890 },
        { name: 'Web Development', members: 280, posts: 650 },
        { name: 'AI/ML', members: 200, posts: 480 },
        { name: 'Mobile Development', members: 150, posts: 320 }
      ]
    };
  }
}
