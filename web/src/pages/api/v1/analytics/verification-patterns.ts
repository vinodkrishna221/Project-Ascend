...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const verificationPatterns = await analyticsService.getVerificationPatternAnalysis(timeWindowHours);

    res.status(200).json({
      success: true,
      data: verificationPatterns,
      meta: {
        timeWindowHours,
    ...
    res.status(500).json({
      ...
      error: {
        ...
        message: 'Failed to retrieve verification pattern analysis'
      }
    });
  }
}
